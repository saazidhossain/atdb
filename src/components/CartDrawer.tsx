...
export default function CartDrawer() {
  ...
  const errors = useMemo(() => {
    ...
    if (location.trim().length < 2)
      e.location = t("Please enter a project location.", "প্রজেক্ট লোকেশন লিখুন।");
    if (location.trim().length > 120)
      e.location = t("Location must be under 120 characters.", "লোকেশন ১২০ অক্ষরের কম হতে হবে।");
    if (!start) e.start = t("Pick a start date.", "শুরুর তারিখ বেছে নিন।");
    else if (start < today)
      e.start = t("Start date can't be in the past.", "শুরুর তারিখ অতীত হতে পারবে না।");
    if (!end) e.end = t("Pick an end date.", "শেষের তারিখ বেছে নিন।");
    else if (start && end < start)
      e.end = t("End date must be on or after start.", "শেষের তারিখ শুরুর পরে হতে হবে।");
    return e;
            ...
            <header className="flex items-center justify-between p-5 border-b border-white/10">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-display font-bold text-lg text-white">
                  {t("Quotation Cart", "কোটেশন কার্ট")}
                </h3>
                <span className="px-2 py-0.5 rounded-full bg-orange-500/20 text-orange-300 text-[10px] font-bold">
                  {count} {t("Unit(s)", "ইউনিট")}
                </span>
              </div>
              <button
                onClick={() => setOpen(false)}
                aria-label={t("Close", "বন্ধ")}
                className="w-9 h-9 rounded-full hover:bg-white/10 flex items-center justify-center text-white/70"
              >
                <X className="w-5 h-5" />
              </button>
            </header>
              ...
              {items.length === 0 ? (
                <div className="text-center py-20">
                  ...
                  <p className="text-white/30 text-xs mt-2">
                    {t(
                      "Add equipment to request a combined quotation.",
                      "একসাথে কোটেশনের জন্য ইকুইপমেন্ট যোগ করুন।",
                    )}
                  </p>
                </div>
              ) : (
                <>
                    ...
                    <div key={it.id} className="glass-card rounded-xl p-3 flex gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-white/50 mt-0.5">
                          {it.brand} · {it.capacity}
                        </p>
                        <div className="flex items-center justify-between mt-2">
                          <div className="flex items-center gap-1 glass rounded-full">
                            <button
                              onClick={() => setQty(it.id, it.qty - 1)}
                              className="w-7 h-7 flex items-center justify-center text-white/70 hover:text-orange-400"
                              aria-label="-"
                            >
                          ...
                          <button
                            ...
                            className="w-7 h-7 flex items-center justify-center text-white/40 hover:text-red-400"
                            aria-label={t("Remove", "মুছুন")}
                          >
                  ...
                  <div className="pt-4 mt-4 border-t border-orange-500/30">
                    <p className="text-[11px] font-bold tracking-[0.24em] text-orange-300 uppercase mb-3">
                      {t("Project Details", "প্রজেক্ট ডিটেইলস")}
                    </p>
                    <label className="block">
                      <span className="flex items-center gap-1.5 text-xs text-white/70 mb-1.5">
                        <MapPin className="w-3 h-3 text-orange-400" /> {t("Location", "লোকেশন")}{" "}
                      ...
                      />
                      {fieldErr("location") && (
                        <span className="mt-1 flex items-center gap-1 text-[11px] text-red-400">
                    ...
                    <div className="grid grid-cols-2 gap-3 mt-3">
                      <label className="block">
                        <span className="flex items-center gap-1.5 text-xs text-white/70 mb-1.5">
                          <CalendarDays className="w-3 h-3 text-orange-400" /> {t("Start", "শুরু")}{" "}
                          <span className="text-orange-400">*</span>
                        </span>
                        <input
                          type="date"
                      ...
                      <label className="block">
                        <span className="flex items-center gap-1.5 text-xs text-white/70 mb-1.5">
                          <CalendarDays className="w-3 h-3 text-orange-400" /> {t("End", "শেষ")}{" "}
                          <span className="text-orange-400">*</span>
                        </span>
                        <input
                          type="date"
                    ...
                    <label className="block mt-3">
                      <span className="flex items-center justify-between text-xs text-white/70 mb-1.5">
                        <span className="flex items-center gap-1.5">
                          <FileText className="w-3 h-3 text-orange-400" /> {t("Note", "নোট")}
                        </span>
                      ...
                      <textarea
                        ...
                        rows={3}
                        placeholder={t(
                          "Site access, working hours, special requirements…",
                          "সাইট অ্যাক্সেস, কাজের সময়, বিশেষ প্রয়োজন…",
                        )}
              ...
              <footer className="p-5 border-t border-white/10 space-y-2 bg-[hsl(220_20%_8%)]">
                {touched && !isValid && (
                  <p className="flex items-center gap-1.5 text-[11px] text-red-400 px-1">
                    <AlertCircle className="w-3 h-3" />
                    {t(
                      "Please complete the required project details above.",
                      "দয়া করে উপরের প্রজেক্ট তথ্যগুলি পূরণ করুন।",
                    )}
                  </p>
                )}
                <a
                  href={
                    isValid
                      ? whatsappCheckoutUrl(safeNote, { location: location.trim(), start, end })
                      : "#"
                  }
                  target="_blank"
                  ...
                  title={
                    isValid
                      ? t(
                          "Send your quotation to ATDB on WhatsApp",
                          "হোয়াটসঅ্যাপে ATDB-কে কোটেশন পাঠান",
                        )
                      : t(
                          "Add Location, Start and End dates to enable WhatsApp checkout.",
                          "WhatsApp চেকআউট চালু করতে লোকেশন, শুরুর ও শেষের তারিখ দিন।",
                        )
                  }
                ...
                <p id="cart-cta-hint" className="sr-only">
                  {isValid
                    ? t(
                        "Opens WhatsApp with your quotation prefilled.",
                        "আপনার কোটেশন প্রি-ফিল করে হোয়াটসঅ্যাপ খুলবে।",
                      )
                    : t(
                        "Disabled until Location, Start and End dates are valid.",
                        "লোকেশন, শুরুর ও শেষের তারিখ ঠিক না করা পর্যন্ত নিষ্ক্রিয়।",
                      )}
                </p>
                <button
                  onClick={clear}
                  className="w-full text-xs text-white/40 hover:text-white/70 py-2"
                >
