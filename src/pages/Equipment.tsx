...
export default function EquipmentPage() {
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-2 gap-5">
            {equipmentCategories.map(cat => (
              <Link key={cat.slug} to={`/equipment/${cat.slug}`} className="group relative overflow-hidden rounded-2xl glass-card glass-hover aspect-[16/10]">
                <img src={cat.image} alt={cat.label} loading="lazy" decoding="async" className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
  ...
  return <CategoryView categorySlug={category} currentCategory={currentCategory} t={t} lang={lang} add={add} />;
}
...
function CategoryView({
  ...
  const search = useSearch({ from: "/equipment/$category/" }) as {
    brand: string; origin: string; year: string; q: string;
  };
  ...
  const brandOptions = useMemo(() => Array.from(new Set(allItems.map(e => e.brand))).sort(), [allItems]);
  const originOptions = useMemo(() => Array.from(new Set(allItems.map(e => e.origin))).sort(), [allItems]);
  const yearOptions = useMemo(
    () => Array.from(new Set(allItems.map(e => e.year).filter(Boolean) as number[])).sort((a, b) => b - a).map(String),
    [allItems]
  );
  ...
  const setFilter = (k: "brand" | "origin" | "year" | "q", v: string) => {
    navigate({ search: (prev: any) => ({ ...prev, [k]: v }), replace: true });
  };
  const clearAll = () => navigate({ search: { brand: "", origin: "", year: "", q: "" }, replace: true });

  const activeCount = ["brand", "origin", "year", "q"].filter(k => (search as any)[k]).length;
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="pt-28 pb-10 relative overflow-hidden">
        {currentCategory && (
          <>
            <img src={currentCategory.image} alt="" loading="lazy" decoding="async" className="absolute inset-0 w-full h-full object-cover opacity-20" />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-white/50 mt-3">
            {items.length}{items.length !== allItems.length ? ` / ${allItems.length}` : ""} {t("units shown", "ইউনিট দেখানো হচ্ছে")} · {currentCategory?.brands}
          </p>
        </div>
      </section>
            ...
            {activeCount > 0 && (
              <div className="mt-3 flex flex-wrap gap-1.5">
                {(["brand", "origin", "year", "q"] as const).map(k => {
                  const v = (search as any)[k] as string;
                  if (!v) return null;
                  return (
                    <button
                      key={k}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {items.length === 0 ? (
            <div className="glass-card rounded-2xl p-10 text-center">
              <p className="text-white/70">{t("No equipment matches your filters.", "আপনার ফিল্টারে কোনো ইকুইপমেন্ট মেলেনি।")}</p>
...
function FilterInput({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string }) {
  return (
    <label className="block">
      <span className="block text-[10px] font-semibold tracking-[0.18em] uppercase text-white/50 mb-1.5">{label}</span>
      <input
        type="text"
        value={value}
  ...
  return (
    <label className="block">
      ...
      >
        ...
        {options.map(o => (
          <option key={o} value={o} className="bg-[hsl(220_20%_10%)]">{o}</option>
        ))}
      </select>
    </label>
  );
