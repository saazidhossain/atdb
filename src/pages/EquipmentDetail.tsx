...
export default function EquipmentDetail() {
      ...
      <div className="min-h-screen bg-background text-foreground flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold mb-4">
            {t("Equipment not found", "যন্ত্রপাতি পাওয়া যায়নি")}
          </h1>
          <Link to="/equipment" className="text-orange-400 hover:underline">
            {t("Back to Equipment", "যন্ত্রপাতিতে ফিরুন")}
          </Link>
        </div>
      </div>
    );
  ...
  const allImages = [...(equipment.realPhotos || []), equipment.image];
      ...
      <section className="pt-24 pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2 text-sm text-white/40 mb-8">
            ...
            >
              <ArrowLeft className="w-3.5 h-3.5" /> {t("Equipment", "যন্ত্রপাতি")}
            </Link>
            <span>/</span>
            <Link
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <div>
              <div className="relative aspect-[4/3] rounded-2xl overflow-hidden glass-card mb-4">
                <img
                  ...
                  fetchPriority="high"
                  loading="eager"
                  className="image-polish w-full h-full object-cover"
                />
                ...
                {equipment.realPhotos && activeImage < equipment.realPhotos.length && (
                  <div className="absolute top-4 right-4 px-3 py-1.5 rounded-full bg-green-600/90 backdrop-blur text-xs font-semibold text-white flex items-center gap-1.5">
                    ...
                    {t("Current Condition", "বর্তমান অবস্থা")}
                  </div>
                )}
              </div>
              {allImages.length > 1 && (
                      ...
                      {!videoPlaying && (
                        ...
                        >
                          <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-orange-500/90 backdrop-blur flex items-center justify-center shadow-2xl hover:scale-110 transition-transform">
                            <Play
                              className="w-7 h-7 sm:w-8 sm:h-8 text-white ml-0.5"
                              fill="white"
                            />
                          </div>
                        </button>
                      ...
                      <div className="absolute bottom-3 right-3 flex gap-2 z-10">
                        {videoPlaying && (
                          <button
                            ...
                            className="w-10 h-10 rounded-full bg-black/50 backdrop-blur flex items-center justify-center text-white active:scale-95"
                          >
                        ...
                        <button
                          ...
                          aria-label={
                            videoMuted ? t("Unmute", "শব্দ চালু") : t("Mute", "শব্দ বন্ধ")
                          }
                          className="w-10 h-10 rounded-full bg-black/50 backdrop-blur flex items-center justify-center text-white active:scale-95"
                        >
                          {videoMuted ? (
                        ...
                        <button
                          ...
                          aria-label={t("Fullscreen", "ফুলস্ক্রিন")}
                          className="w-10 h-10 rounded-full bg-black/50 backdrop-blur flex items-center justify-center text-white active:scale-95"
                        >
            ...
            <div>
              <div className="flex items-center gap-3 mb-3">
                <span className="px-3 py-1 rounded-full glass text-xs font-mono text-white/60">
                  {equipment.id}
                </span>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                {[
                  ...
                  { icon: MapPin, label: t("Origin", "উৎপত্তি"), value: equipment.origin },
                  {
                    icon: Calendar,
                    label: t("Year", "সাল"),
                    value: equipment.year ? String(equipment.year) : "—",
                  },
                ...
                <table className="w-full text-sm table-fixed">
                  <tbody className="divide-y divide-white/5">
                    {[
                      ...
                      {
                        k: t("Lifting / Operating Capacity", "লিফটিং / অপারেটিং ক্ষমতা"),
                        v: equipment.capacity,
                      },
                      {
                        k: t("Engine / Power", "ইঞ্জিন / পাওয়ার"),
                        v: `${equipment.fuel}${equipment.notes ? ` · ${equipment.notes}` : ""}`,
                      },
                      { k: t("Country of Origin", "উৎপত্তির দেশ"), v: equipment.origin },
                      {
                        k: t("Year of Manufacture", "নির্মাণ সাল"),
                        v: equipment.year ? String(equipment.year) : "—",
                      },
                      { k: t("Brand", "ব্র্যান্ড"), v: equipment.brand },
                      { k: t("Asset ID", "অ্যাসেট আইডি"), v: equipment.id },
                      {
                        k: t("Quantity Available", "উপলব্ধ পরিমাণ"),
                        v: `${equipment.quantity} ${t("Unit(s)", "ইউনিট")}`,
                      },
                    ].map((row) => (
                      <tr key={row.k} className="align-top">
                        <th
                          scope="row"
                          className="text-left px-4 sm:px-5 py-2.5 text-white/50 font-normal w-[46%] break-words"
                        >
                          {row.k}
                        </th>
                        <td className="px-4 sm:px-5 py-2.5 text-white font-semibold break-words">
                          {row.v}
                        </td>
                      </tr>
                    ))}
              ...
              <div className="space-y-3">
                ...
                >
                  <Download
                    className={`w-5 h-5 flex-shrink-0 ${pdfLoading ? "animate-bounce" : ""}`}
                  />
                ...
                <button
                  onClick={() => {
                    add({
                      ...
                      image: equipment.realPhotos?.[0] || equipment.image,
                    });
                    setCartOpen(true);
                  }}
                ...
                >
                  ...
                  <span className="min-w-0">
                    {t(
                      "Add to Rental Cart — review before WhatsApp",
                      "ভাড়ার কার্টে যোগ করুন — WhatsApp-এর আগে রিভিউ",
                    )}
                  </span>
                </button>
